...
import('https://raw.githubusercontent.com/adionvalorant-crypto/BunkHelper/2f75cbfc571091a1c8e4ba96efcd282febc133e2/app-fixed.js').then(async()=>{const{getAuth,getRedirectResult,signInWithRedirect,GoogleAuthProvider}=await import('https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js');const auth=getAuth();const login=document.getElementById('login-btn');if(!login)return;login.onclick=async()=>{login.disabled=true;login.textContent='Signing in…';try{await signInWithRedirect(auth,new GoogleAuthProvider())}catch(e){console.error('Google sign-in failed',e);login.disabled=false;login.textContent='Continue with Google';toast('Login failed: '+(e.code||e.message),'error')}};try{await getRedirectResult(auth)}catch(e){console.error('Google redirect sign-in failed',e);login.disabled=false;login.textContent='Continue with Google';toast('Login failed: '+(e.code||e.message),'error')}});
