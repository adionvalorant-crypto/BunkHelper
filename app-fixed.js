...
import('https://raw.githubusercontent.com/adionvalorant-crypto/BunkHelper/2f75cbfc571091a1c8e4ba96efcd282febc133e2/app-fixed.js').then(async()=>{const{getAuth,setPersistence,browserLocalPersistence,signInWithPopup,GoogleAuthProvider}=await import('https://www.gstatic.com/firebasejs/10.8.0/firebase-auth.js');const auth=getAuth();const login=document.getElementById('login-btn');if(!login)return;const showAuthError=e=>{console.error('Google sign-in failed',e);login.disabled=false;login.textContent='Continue with Google';let box=document.getElementById('auth-error');if(!box){box=document.createElement('div');box.id='auth-error';box.style.cssText='margin-top:14px;padding:10px 12px;border:1px solid rgba(255,102,127,.35);border-radius:10px;color:#ff9aaa;background:rgba(255,102,127,.07);font-size:12px;line-height:1.5;word-break:break-word';login.insertAdjacentElement('afterend',box)}box.textContent='Google sign-in failed: '+(e?.code||e?.message||'Unknown error');};login.onclick=async()=>{login.disabled=true;login.textContent='Opening Google…';document.getElementById('auth-error')?.remove();try{await setPersistence(auth,browserLocalPersistence);await signInWithPopup(auth,new GoogleAuthProvider())}catch(e){showAuthError(e)}}}).catch(e=>{console.error('BunkHelper startup failed',e);const login=document.getElementById('login-btn');if(login){login.disabled=false;login.textContent='Continue with Google';const box=document.createElement('div');box.style.cssText='margin-top:14px;padding:10px 12px;border:1px solid rgba(255,102,127,.35);border-radius:10px;color:#ff9aaa;background:rgba(255,102,127,.07);font-size:12px;line-height:1.5;word-break:break-word';box.textContent='App startup failed: '+(e?.message||e);login.insertAdjacentElement('afterend',box)}});